...
#FIXME
d %<>% filter(date > as.Date("2020-09-20"))


d %<>% mutate(url = str_c("https://www.congress.gov/116/crec/", 
                          date %>% str_replace_all("-", "/"), 
                          "/modified/", 
                          file))
#/FIXME
...
```{r}
d %>% mutate(sentence = map_chr(file, .f = keyword_sentence, word = "census") ) %>%
  filter(sentence != "NA") %>% 
  select(date, sentence, url) %>% kablebox()
```
...
```{r}
d %>% mutate(sentence = map_chr(file, .f = keyword_sentence, word = "redistrict|gerrymander|efficiency gap") ) %>%
  filter(sentence != "NA") %>% 
  select(date, sentence, url) %>% kablebox()
```
...
Sentences containing "partisan bias" between `r min(d$date)` and `r max(d$date)`
```{r}
d %>% mutate(sentence = map_chr(file, .f = keyword_sentence, word = "partisan bias") ) %>%
  filter(sentence != "NA") %>% 
  select(date, sentence, url) %>% kablebox()
```